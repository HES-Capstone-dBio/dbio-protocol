...
use crate::db::Db;
use crate::errors::{ServerError, ServerError::*};
use crate::models::*;
use crate::StdErr;
...
fn adapt_server_error(e: ServerError) -> Error {
    match e {
        DatabaseError(e) =>
        // TODO: Adapt different SQL errors to status codes
        {
...
async fn add_user(db: Data<Db>, user: Json<User>) -> Result<Json<User>, Error> {
...
async fn get_user_from_email(db: Data<Db>, Path(email): Path<String>) -> Result<Json<User>, Error> {
    db.get_user_by_email(email)
        .await
        .map(Json)
        .map_err(adapt_server_error)
}
...
async fn post_resource_data(
    ...
    let cid: String = {
        hasher.write(data.ciphertext.as_bytes());
        hasher.finish().to_string()
    };
    db.get_user_by_email(data.email)
        .and_then(|user| db.insert_resource_data(ResourceData(cid, data.ciphertext)))
        .await
        .map_err(adapt_server_error)
}
